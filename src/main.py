...
def sling_action():
    ...
    uv = unit_vector(v)
    # vecteur unitaire X (uv1) et y (UV2) du référentiel a la sourie
    uv1 = uv[0]
    uv2 = uv[1]
    # calcule de la distance entre reférentiel et la sourie
    ...
    # pour pas que la rope dépasse sa longueur
    ...
    if toggle_draw_options:
        ...
        # sens du vecteur inverse
        i = 1
        if x_mouse > sling_x:
            i = -1

        # position x du vecteur inverse
        linex = i*(sling_x + (dist*math.cos(angle))+5)
        liney = 0
        
        if x_mouse > sling_x:
            linex = (sling_x - (dist*math.cos(angle))+5)
            if i == 1:
                liney = sling_y + (dist*math.sin(angle+math.pi))+5
            else:
                liney = sling_y - (dist*math.sin(angle))+5
        else:
            # position y du vecteur inverse
            if i == 1:
                liney = sling_y + (dist*math.sin(angle))+5
            else:
                liney = sling_y - (dist*math.sin(angle))+5
        ...
        # affiche le vecteur inverse
        pygame.draw.line(screen, (0, 255, 0), (sling_x, sling_y), (linex, liney), 2)
        pygame.draw.line(screen, (255, 255, 255), (sling_x, sling_y),
                         (x_mouse, y_mouse), 1)
        pygame.draw.circle(screen, (255, 0, 0), (int(sling_x), int(sling_y)),
                           5, 0)
        pygame.draw.circle(screen, (255, 0, 0), (int(x_mouse), int(y_mouse)),
                           5, 0)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if not toggle_draw_options:
                toggle_draw_options = True
                redbird = pygame.image.load(CIRCLE_IMG).convert_alpha()
                pig_happy = pygame.image.load(ENEMY_IMG).convert_alpha()
                background2 = pygame.image.load(BLANK_IMG).convert_alpha()
                full_sprite = pygame.image.load(ENEMY_SPRITE_IMG).convert_alpha()
                rect = pygame.Rect(181, 1050, 50, 50)
                cropped = full_sprite.subsurface(rect).copy()
                pig_image = pygame.transform.scale(cropped, (30, 30))
                sling_image = pygame.image.load(SL_IMG).convert_alpha()
                debug_toggle_col_and_beams()
